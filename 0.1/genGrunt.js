...
genGrunt.generate = (function() {
    ...
    var commands = {
            ...
            help_gengrunt: function(args, msg) {
              sendChat(`genGrunt v${genGrunt.scriptVersion}`, 'Specify !genGrunt followed by the type to generate (Citizen, Police, Military, NationalGuard, Ganger, or Survivor).');
            }
        };

    function makeGrunt(playerID, playerName, gruntType) {
        var myGender = getGender(gruntType);
        var myName = getName(gruntType, myGender);
    ...
    function getGender(gruntType) {
        ...
        switch(gruntType){
            case "Citizen":
                ...
                break;
            case "Police":
                if (chance < 18) { myGender = "Female"; } else { myGender = "Male"; }
                break;
            case "Military":
                if (chance < 21) { myGender = "Female"; } else { myGender = "Male"; }
                break;
            case "NationalGuard":
                if (chance < 13) { myGender = "Female"; } else { myGender = "Male"; }
                break;
            case "Ganger":
    ...
    function getName(gruntType, myGender) {
        var chance = randomInteger(100),
            myName;
        switch(myGender){
            case "Male":
    ...
    function getAge(gruntType) {
        ...
        switch(gruntType){
            ...
            case "NationalGuard":
                if (chance < 66) {
                    myAge = 21 + randomInteger(18);
                } else {
                    myAge = 39 + randomInteger(15);
                }
                break;
            case "Ganger":
                if (chance < 13) {
